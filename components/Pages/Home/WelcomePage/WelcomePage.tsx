  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        if (p.flake) {
          ctx.strokeStyle = '#0f766e'; ctx.lineWidth = 0.6;
          ctx.translate(p.x, p.y);
          for (let i = 0; i < 6; i++) { ctx.rotate(Math.PI / 3); ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(0, p.r * 3.2); ctx.stroke(); }
        } else { ctx.fillStyle = '#0f766e'; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fill(); }
        ctx.restore();
        p.x += p.vx; p.y += p.vy;
...
const CHIPS = ['24/7 Emergency', 'No Contracts', 'Licensed Locksmith', '20+ Yrs Local', 'Workmanship Guaranteed'];
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.content}>
          <motion.div className={styles.badge}
            ...
            Waco&apos;s Most Trusted Locksmith — Since 2005
          </motion.div>

          <motion.h1 className={styles.headline}
            initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.1 }}>
            Locked Out.<br />Keys Lost.<br />
            <span className={styles.accentLine}>Keyline Locksmith.</span>
          </motion.h1>

          <motion.p className={styles.sub}
            initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.55, delay: 0.22 }}>
            24/7 Lockouts · Locks · Keys · Security. Flat-rate pricing. Fast emergency response.
            Workmanship Guaranteed. Serving Waco and Central Texas with licensed, bonded locksmiths.
          </motion.p>

          <motion.div className={styles.ctaRow}
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.34 }}>
            <a href="tel:+12548202020" className={styles.ctaPrimary}>
